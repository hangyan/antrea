...
func (c *Controller) uploadToFileServer(up uploader, uid string, parsedURL *url.URL, serverAuth *controlplane.BundleServerAuthConfiguration, packetFile io.Reader) error {
	joinedPath := path.Join(parsedURL.Path, c.nodeConfig.Name+"_"+uid+".pcapng")
...
func (c *Controller) uploadPackets(ps *crdv1alpha1.PacketSampling, outputFile afero.File) error {
	...
	for triesLeft > 0 {
		if uploadErr = c.uploadToFileServer(uploader, string(ps.UID), parsedURL, authentication, outputFile); uploadErr == nil {
			return nil
		}
		triesLeft--
		if triesLeft == 0 {
