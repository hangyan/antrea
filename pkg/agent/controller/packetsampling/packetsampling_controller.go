...
func (c *Controller) initPacketSampling(ps *crdv1alpha1.PacketSampling) error {
	...
	err = c.updatePacketSamplingStatus(ps, crdv1alpha1.PacketSamplingRunning, "")
	if err != nil {
...
func (c *Controller) updatePacketSamplingStatus(ps *crdv1alpha1.PacketSampling,
	phase crdv1alpha1.PacketSamplingPhase, reason string) error {
	update := ps.DeepCopy()
	...
	_, err := c.crdClient.CrdV1alpha1().PacketSamplings().UpdateStatus(context.TODO(), update, metav1.UpdateOptions{})
	return err
}
...
func (c *Controller) checkPacketSamplingStatus(ps *crdv1alpha1.PacketSampling) error {
	...
	if checkPacketSamplingSucceeded(ps) {
		c.deallocateTag(ps.Name, tag)
		return c.updatePacketSamplingStatus(ps, crdv1alpha1.PacketSamplingSucceeded, "")
	}

	if checkPacketSamplingTimeout(ps) {
		c.deallocateTag(ps.Name, tag)
		return c.updatePacketSamplingStatus(ps, crdv1alpha1.PacketSamplingFailed, samplingTimeoutReason)
	}
	return nil
}
