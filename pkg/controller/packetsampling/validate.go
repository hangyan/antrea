...
func validate(ps *crdv1alpha1.PacketSampling) (allowed bool, deniedReason string) {
	...
	if ps.Spec.Type != crdv1alpha1.FirstNSampling {
		return false, fmt.Sprintf("PacketSampling %s has invalid type %s, supported type is [%s]", ps.Name, ps.Spec.Type, crdv1alpha1.FirstNSampling)
	}
